...
export function InstallButton() {
  ...
  if (installed) return <span className="inline-flex h-10 items-center rounded-full border border-line px-4 text-sm text-ink-2">{d.share.setup.installed}</span>;
  return (
    <button
      type="button"
      disabled={!evt}
      title={evt ? "" : d.share.setup.installHint}
  ...
  return (
    <span className="mt-1 flex items-center gap-2">
      ...
      <button type="button" onClick={async () => { try { await navigator.clipboard.writeText(value); setDone(true); setTimeout(() => setDone(false), 1500); } catch { /* ignore */ } }} className="text-[12px] text-ink-2 hover:text-ink">
        {done ? d.share.setup.copied : d.share.setup.copy}
      </button>
    </span>
  );
}
