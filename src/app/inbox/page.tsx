...
export default async function InboxPage() {
  ...
  return (
    <div className="space-y-10">
      ...
      {failed.length > 0 && (
        <section>
          <h2 className="eyebrow mb-3">{d.inbox.failed} · {failed.length}</h2>
      ...
      <section>
        <h2 className="eyebrow mb-3">{d.inbox.recent} · {recent.length}</h2>
        <ReferenceGrid references={recent.map(toPublic)} lang={lang} empty={d.inbox.nothing48h} />
      </section>
